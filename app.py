...
def validate_ai_response_sections(text, headers):
    missing = [header for header in headers if header not in text]
    if missing:
        print(f"[validate_ai_response_sections] Missing headers: {missing}")
    return missing


def extract_section(text, header):
    pattern = re.compile(rf"^{re.escape(header)}\s*(.*?)(?=^###\s|\Z)", re.S | re.M)
    match = pattern.search(text)
    if match:
        return match.group(1).strip()
    print(f"[extract_section] Header not found or parse failed: {header!r}")
    return f"Unable to extract the '{header}' section. The AI output may have drifted from the expected format."

SAMPLE_AI_RESPONSE = """### RESUME SUMMARY
Experienced marketing professional with 7 years of expertise in digital campaign management and analytics.
...
### CRITICAL WEAKNESSES
- Limited experience with BI platform integrations
- Resume could better highlight measurable results

### INTERVIEW PREPARATION
- Describe a time you improved campaign performance...

### RESUME REWRITE SUGGESTIONS
- Clarify impact statements with metrics and outcomes

### CAREER COACH GUIDANCE
- Gain hands-on experience with BI tools like Tableau or Looker

### TAILORED COVER LETTER
Dear Hiring Manager,

I am excited to apply for the marketing manager role...
"""


def run_sample_parser_test():
    missing_headers = validate_ai_response_sections(SAMPLE_AI_RESPONSE, AI_OUTPUT_HEADERS)
    print("Missing headers:", missing_headers)
    for header in AI_OUTPUT_HEADERS:
        print(f"{header}: {extract_section(SAMPLE_AI_RESPONSE, header)!r}\n")
...
def run_app():
    ...
    if not profile:
        st.error("User profile not found. Please contact support or try logging in again.")
        st.stop()
    ...
    if st.button("Analyze Resume", type="primary"):
        if not resume or not job1:
            ...
            with st.spinner("🤖 Remote Pilot AI is scanning your credentials and simulating ATS filtering..."):
                ...
                Return the entire response exactly in the following sections with these headers. Do not add extra top-level headings or nested headers outside the specified output structure.
                ...
                try:
                    ...
                    missing_headers = validate_ai_response_sections(ai_text, AI_OUTPUT_HEADERS)
                    if missing_headers:
                        st.warning(
                            "The AI output may have skipped or reformatted some expected sections. "
                    ...
                    st.session_state.analysis_results = {
                        ...
                        "summary": extract_section(ai_text, "### RESUME SUMMARY"),
                        "strengths": extract_section(ai_text, "### CORE STRENGTHS"),
                        "weaknesses": extract_section(ai_text, "### CRITICAL WEAKNESSES"),
                        "interview": extract_section(ai_text, "### INTERVIEW PREPARATION"),
                        "rewrite": extract_section(ai_text, "### RESUME REWRITE SUGGESTIONS"),
                        "coach": extract_section(ai_text, "### CAREER COACH GUIDANCE"),
                        "cover_letter": extract_section(ai_text, "### TAILORED COVER LETTER")
                    }
    ...
    if st.sidebar.button("Test Gemini Connection"):
        if not gemini_enabled:
            st.sidebar.error("Gemini is not configured correctly. Check your API key in .streamlit/secrets.toml.")
        else:
            try:
